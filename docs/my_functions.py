...
def load_headers(file):
    ...
    for row in raw_data: 
        if check_header(file) == True and len(header) == 0:
            header = row
            header = header.split()
        else:    
            row = row.replace(",", ".")
...
def plotpearson(data, labels, cm="seismic", fons=20, figs=(20, 17), tfs=25, ti="Pearson"):
    """
...
def plotspearman(data, labels, cm="seismic", fons=20, figs=(20, 17), tfs=25, ti="Spearman"):
    """
    Calculates Pearson matrix and plots it.
...
def plotgrau(data, labels, cm="seismic", fons=20, figs=(25, 15),
    ...
    graus = grau(data) # grau correlation (3d R2)
