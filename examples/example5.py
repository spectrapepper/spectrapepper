...
# load data to fit
y = spep.load_spectras()[10] # load 1 single spectra from the data
y = spep.normtomax(y) # Normalize the maximum value to 1

# load axis
x = spep.load_spectras()[0] # load the x axis from the dataset

# select peak to fit to
peak = 206 # aproximate position of the peak (in cm-1) to be evaulated
...
lorentz = spep.lorentzfit(y=y, x=x, pos=peak+31, gamma=5, manual=True)
student = spep.studentfit(y=y, x=x, pos=peak+31, v=0.1, manual=True)
...
for i,j in zip([gauss, lorentz, student], ['Gauss','Lorentz','Student']):
